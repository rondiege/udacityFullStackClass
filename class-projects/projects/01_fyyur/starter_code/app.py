...
def venues():
    ...
    # HI udacity! This is the query I want.... how do you do it in Alchemy? I've spent way to long tinkering on it
    #  SELECT name, (SELECT count(*) FROM shows WHERE shows.date >= NOW() AND venues.id = shows.venue_id) as num_shows FROM venues;
...
def show_artist(artist_id):
    ...
    comingShows = Show.query.join(Venue).filter(Show.start_time > datetime.now()).add_entity(Venue).all()
    pastShows = Show.query.join(Venue).filter(Show.start_time < datetime.now()).add_entity(Venue).all()
